...
function find_word_concatenation(str, words) {
    ...
    // Step 1:  Keep the frequency of every word in a HashMap.
    wordFrequency = {};
    ...
    // Step 2: Starting from every index in the string, try to match all the words.
    const resultIndices = [],
      wordsCount = words.length;
    wordLength = words[0].length;
  
    // Step 3: In each iteration, keep track of all the words that we have already seen in another HashMap.
    for (i = 0; i < (str.length - wordsCount * wordLength) + 1; i++) {
      const wordsSeen = {};
      // Step 4: If a word is not found or has a higher frequency than required, we can move on to the next character in the string.
      for (j = 0; j < wordsCount; j++) {
        ...
        if (j + 1 === wordsCount) { // Step 5: Store index if we have found all the words
  ...
  console.log(find_word_concatenation('catcatfox', ['cat', 'fox']));